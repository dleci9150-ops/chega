...
/**
 * Middleware para auto-descriptografar mensagens antes do handler
 */
function encryptionMiddleware(req, res, next) {
  ...
  res.json = function(data) {
    ...
    if (data && data.messages && Array.isArray(data.messages)) {
      // Auto-descriptografar se chave disponível
...
function logCryptoOperation(req, res, next) {
  ...
  res.json = function(data) {
    if (data.success) {
      ChatEncryptionService.logCryptoOperation(
        req.user?.id,
 ...
 */
function requireHttpsForCrypto(req, res, next) {
